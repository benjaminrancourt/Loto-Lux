 ...
 (function($) {
	...
	Calendar.prototype = {
		...
		_render: function() {
			...
			this._renderYearHeader();
			if(this.options.displayMonthHeader) {
		...
		_renderYearHeader: function() {
			...
			if(!this.options.disablePrev2Year) {
			...
			if(!this.options.disableNext2Year) {
		...
		_renderMonthHeader: function() {
			var header = $(document.createElement('div'));
			header.addClass('calendar-header-month panel panel-default');

			var headerTable = $(document.createElement('table'));

			var firstDisplayedMonth = 0;
			var currentMonth = this.options.startMonth;
			var lastDisplayedMonth = 12;

			if(this.options.numDisplayedMonthHeader < 12) {
				firstDisplayedMonth = this.options.startMonth - Math.floor(this.options.numDisplayedMonthHeader / 2);

				if(firstDisplayedMonth < 0) {
					firstDisplayedMonth = 0;
				}

				lastDisplayedMonth = firstDisplayedMonth + this.options.numDisplayedMonthHeader;

				if(lastDisplayedMonth > 12) {
					lastDisplayedMonth = 12;
					firstDisplayedMonth = lastDisplayedMonth - this.options.numDisplayedMonthHeader;
				}
			}
			...
			if(firstDisplayedMonth == 0 || (this.options.minDate != null &&
				this.options.minDate > new Date(this.options.startYear, this.options.startMonth - 1, 31))) {
				prevDiv.addClass('disabled');
			}
			...
			for(var m = firstDisplayedMonth; m < lastDisplayedMonth; m++) {
				var monthDiv = $(document.createElement('th'));
				monthDiv.addClass('month-title');
				monthDiv.text(dates[this.options.language].monthsShort[m]);

				var lastDay = new Date(this.options.startYear, m + 1, 1);
				lastDay.setDate(lastDay.getDate() - 1);
				var firstDay = new Date(this.options.startYear, m, 1);

				if(this.options.minDate != null && this.options.minDate > lastDay) {
					monthDiv.addClass('disabled');
				}

				if(this.options.maxDate != null && this.options.maxDate < firstDay) {
					monthDiv.addClass('disabled');
				}

				if(currentMonth != m) {
					var diffMonth = Math.abs(currentMonth - m);
					monthDiv.addClass('month-neighbor');
					monthDiv.addClass('hidden-xs');
					monthDiv.addClass('month-neighbor-' + diffMonth);

					if(diffMonth > 1) {
						monthDiv.addClass('hidden-sm');
					}
				}

				headerTable.append(monthDiv);
			}

			var nextDiv = $(document.createElement('th'));
			nextDiv.addClass('next');

			if(lastDisplayedMonth == 12 || (this.options.maxDate != null
				&& this.options.maxDate < new Date(this.options.startYear, this.options.startMonth + 1, 1))) {
				nextDiv.addClass('disabled');
			}
		...
		_renderBody: function() {
			...
			if(this.options.numDisplayedMonth < 12) {
				firstDisplayedMonth = currentMonth - Math.floor(this.options.numDisplayedMonth / 2);

				if(firstDisplayedMonth < 0) {
					firstDisplayedMonth = 0;
				}

				lastDisplayedMonth = firstDisplayedMonth + this.options.numDisplayedMonth;

				if(lastDisplayedMonth > 12) {
					lastDisplayedMonth = 12;
					firstDisplayedMonth = lastDisplayedMonth - this.options.numDisplayedMonth;
				}
			}
			...
			for(var m = firstDisplayedMonth; m < lastDisplayedMonth; m++) {
				...
				if(diffMonth == 0) {
					monthDiv.addClass('not-hidden');
				} else if(this.options.displayMonthHeader) {
					if(diffMonth == 1) {
						monthDiv.addClass('hidden-xs');
					} else if (diffMonth > 1) {
						monthDiv.addClass('hidden-xs');
				...
				while(currentDate <= lastDate)
				{
					...
					do
					{
						...
						if(currentDate < firstDate) {
							...
							else if(this.options.enabledDays.length > 0) {
								cell.addClass('disabled');
								for(var d in this.options.enabledDays){
									if(currentDate.getTime() == this.options.enabledDays[d].getTime()) {
		...
		_applyEvents: function () {
			...
			this.element.find('.year-neighbor, .year-neighbor2').click(function() {
				if(!$(this).hasClass('disabled')) {
					...
					if(_this.getYear() < newYear) {
						_this.setMonth(0);
					} else {
						_this.setMonth(11);
					}
					_this.setYear(newYear);
				}
			});

			this.element.find('.month-neighbor').click(function() {
				if(!$(this).hasClass('disabled')) {
					var newMonth = dates[_this.options.language].monthsShort.indexOf($(this).text());
					_this.setMonth(newMonth);
				}
			});
			...
			this.element.find('.calendar-header-month .prev').click(function() {
				if(!$(this).hasClass('disabled')) {
					_this.element.find('.months-container').animate({'margin-left':'100%'},100, function() {
						_this.element.find('.months-container').hide();
						_this.element.find('.months-container').css('margin-left', '0');
			...
			this.element.find('.calendar-header-month .next').click(function() {
				if(!$(this).hasClass('disabled')) {
					_this.element.find('.months-container').animate({'margin-left':'-100%'},100, function() {
						_this.element.find('.months-container').hide();
						_this.element.find('.months-container').css('margin-left', '0');
			...
			var previousCalendarSize = 0;
			var previousMonthSize = 0;

			setInterval(function() {
				var calendarSize = $(_this.element).width();
				var monthSize = $(_this.element).find('.not-hidden').find('.month').first().width() + 10;
				var numMonth = _this.options.numDisplayedMonth;

				if(previousCalendarSize == calendarSize && previousMonthSize == monthSize) {
					return;
				}

				previousCalendarSize = calendarSize;
				previousMonthSize = monthSize;

				var monthContainerClass = 'month-container';
				var monthContainer = $(_this.element).find('.month-container');

				if(numMonth >= 6 && monthSize * 6 < calendarSize) {
					monthContainerClass = 'col-xs-2';
				}
				else if(numMonth >= 4 && monthSize * 4 < calendarSize) {
					monthContainerClass = 'col-xs-3';
				}
				else if(numMonth >= 3 && monthSize * 3 < calendarSize) {
					monthContainerClass = 'col-xs-4';
				}
				else if(numMonth >= 2 && monthSize * 2 < calendarSize) {
				...
				if(_this.options.displayMonthHeader) {
					var hiddenXS = monthContainer.filter('.hidden-xs');
					var hiddenSM = hiddenXS.filter('.hidden-sm');

					monthContainer = monthContainer.not(hiddenXS);
					hiddenXS = hiddenXS.not(hiddenSM);

					hiddenSM.attr('class', monthContainerClass + ' hidden-xs hidden-sm');
					hiddenXS.attr('class', monthContainerClass + ' hidden-xs');
				}

				monthContainer.attr('class', monthContainerClass + ' not-hidden');
			}, 300);
		},
		...
		getNumDisplayedMonth: function() {
			return this.options.numDisplayedMonth;
		},
		setNumDisplayedMonth: function(num) {
			var parsedNum = parseInt(num);
			if(!isNaN(parsedNum)) {
				this.options.numDisplayedMonth = parsedNum;
				this._render();
			}
		},
		getNumDisplayedMonthHeader: function() {
			return this.options.numDisplayedMonthHeader;
		},
		setNumDisplayedMonthHeader: function(num) {
			var parsedNum = parseInt(num);
			if(!isNaN(parsedNum)) {
				this.options.numDisplayedMonthHeader = parsedNum;
				this._render();
			}
		},
		getDisplayMonthHeader: function() {
			return this.options.displayMonthHeader;
		},
		setDisplayMonthHeader: function(displayMonthHeader) {
			this.options.displayMonthHeader = displayMonthHeader;
			this._render();
		},
		getMinDate: function() {
		...
		setEnabledDays: function(enabledDays) {
		...
		getDisableNext2Year: function() {
			return this.options.disableNext2Year;
		},
		setDisableNext2Year: function(disableNext2Year) {
			this.options.disableNext2Year = disableNext2Year;
			this._render();
		},
		getDisablePrev2Year: function() {
			return this.options.disablePrev2Year;
		},
		setDisablePrev2Year: function(disablePrev2Year) {
			this.options.disablePrev2Year = disablePrev2Year;
			this._render();
		},
